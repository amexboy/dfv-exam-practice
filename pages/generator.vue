...
//   components: { Question },
  ...
  computed: {
    matches () {
      const pattern = /((?:Aerodynamics)|(?:Free ?fall)|(?:Air traffic law)|(?:Meteorology)|(?:Equipment)|(?:Human Performance)|(?:Behaviour in special circumstances)).*?Question No\.? ([0-9]+)(.*?)(\(.*?\)).*?a\)(.*?)b\)(.*?)c\)(.*?)d\)(.*?)Correct answer: ([a-e])/sigm
